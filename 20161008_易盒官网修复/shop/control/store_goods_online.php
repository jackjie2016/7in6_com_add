...
class store_goods_onlineControl extends BaseSellerControl {
	...
	public function copy_goodsOp() {
			$common_id = $_GET['commonid'];	
			$goods_id_max = Model()->table('goods')->field('goods_id')->order('goods_id desc')->limit(1)->find();
			$goods_info_copy = Model()->table('goods')->where(array('goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']))->find();
			$common_goods_id_max = Model()->table('goods_common')->field('goods_commonid')->order('goods_commonid desc')->limit(1)->find();
			$common_goods_info_copy = Model()->table('goods_common')->where(array('goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']))->find();
			$goods_info_copy['goods_id']=$goods_id_max['goods_id'] + 1;
			$common_goods_info_copy['goods_commonid']=$common_goods_id_max['goods_commonid'] + 1;
			$goods_info_copy['goods_commonid']=$common_goods_info_copy['goods_commonid'];
			$goods_add_time = strtotime(date('Y-m-d H:i:s'));
			$goods_info_copy['goods_edittime']=$goods_add_time;	
			$goods_info_copy['goods_addtime']=$goods_add_time;				
			$common_goods_info_copy['goods_addtime']=$goods_add_time;				
			$common_goods_info_copy['goods_selltime']=$goods_add_time;				
			$model_goods = Model('goods');			
			$model_goods->addGoods($goods_info_copy);				
			$model_goods_common = Model('goods_common');				
			$model_goods_common->insert($common_goods_info_copy);			
			header("Location:".urlShop('store_goods_online', 'index'));		   
			$this->indexOp();   
	}	
    ...
    public function edit_goodsOp() {
        ...
        if (intval($_GET['class_id']) > 0) {
		Tpl::output('goods_class', Model('goods_class')->getGoodsClassLineForTag($_GET['class_id']));
        }else{
		Tpl::output('goods_class', $goods_class);
	}
		...
		echo "<script>console.log(".json_encode($goodscommon_info['gc_id']).")</script>";
	...
	$update_common['goods_images_array'] = $_POST['goods_images_array'];
        ...
        $update_common['goods_min_order']    = $_POST['g_min_order'];
        $update_common['goods_per_plus']     = $_POST['g_per_plus'];
        $update_common['goods_attr']         = serialize($_POST['attr']);
        $update_common['goods_body']         = $_POST['g_body'];
        
        //微信三级返利插件start
        $update_common['rank1']         =  floatval($_POST['rank1']);
        $update_common['rank2']         =  floatval($_POST['rank2']);
        $update_common['rank3']         =  floatval($_POST['rank3']);
        //微信三级返利插件end 
        ...
        $update_common['goods_selltime']     = strtotime($_POST['starttime']) + intval($_POST['starttime_H'])*3600 + intval($_POST['starttime_i'])*60;				$update_common['purchase_quantity']	 = $_POST['quota'];
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['goods_min_order']=    $update_common['goods_min_order'];
                    $update['goods_per_plus']=     $update_common['goods_per_plus'];
                    ...
                    //微信三级返利插件start
                    $update['rank1']        = $update_common['rank1'];
                    $update['rank2']        = $update_common['rank2'];
                    $update['rank3']        = $update_common['rank3'];										$update['purchase_quantity']        = $update_common['purchase_quantity'];
                    //微信三级返利插件end
...
// 虚拟商品不能有赠品
			...
			$upload->save2($save_file);
                    ...
                    $insert['goods_min_order']=    $update_common['goods_min_order'];
                    $insert['goods_per_plus']=     $update_common['goods_per_plus'];
                    $insert['goods_spec']        = serialize($value['sp_value']);
                    $insert['goods_storage']     = $value['stock'];
                    $insert['goods_image']       = $update_common['goods_image'];
		    $insert['goods_images_array'] = $update_common['goods_images_array'];
                   ...
                   //微信三级返利插件start
                    $insert['rank1']        = $update_common['rank1'];
                    $insert['rank2']        = $update_common['rank2'];
                    $insert['rank3']        = $update_common['rank3'];
                    //微信三级返利插件end	  $insert['purchase_quantity']        = $update_common['purchase_quantity'];
                    $insert['goods_virtual_salenum'] = rand (0,10);
                    //$goods_id = $model_goods->addGoods($insert);
                    $goods_id = $model_goods->table(goods)->addGoods($insert);
		...
		$insert['goods_images_array']       = $update_common['goods_images_array'];
                ...
                $insert['rank1']        = $update_common['rank1'];
                $insert['rank2']        = $update_common['rank2'];
                $insert['rank3']        = $update_common['rank3'];										$insert['purchase_quantity']        = $update_common['purchase_quantity'];
                //微信三级返利插件end
        ...
        if (!empty($img_array)) {
            foreach ($img_array as $val) {
                if (isset($image_list[$val['color_id']])) {
                    ...
                    $image_array[$val['color_id']][0]['goods_image'] = !empty($val['goods_image'])?$val['goods_image']:$goods_images_array[$i];
