...
class buy_1Logic {
	    ...
	    switch ($preferential_type) {
	    	...
	    	case 'voucher':
	    	    ...
	    	    foreach ($preferential_array as $store_id => $voucher_info) {
	    	        ...
	    	        $store_goods_total[$store_id] -= $voucher_info[0]['voucher_price'];
                        $store_goods_total[$store_id] -= $voucher_info[1]['voucher_price'];
                        $store_goods_total[$store_id] -= $voucher_info[2]['voucher_price'];
                        
                        /*增加判断当总额为负数的情况*/
                        if($store_goods_total[$store_id]<0){
                            
                            $store_goods_total[$store_id]=0;
	...
	public function getStoreAvailableVoucherList($store_goods_total, $member_id,$store_cart_list) {
            ...
            $store_class = $model->table('goods_class')->where("gc_parent_id=0")->field('gc_id')->select();
            
            $new_class=array();
            
            foreach($store_class as $v => $vo){
                
                foreach($store_cart_list as $v1 =>$vo1){
                    
                    $jiage = 0;
                    
                    foreach($vo1 as $v1=>$vo2){
                       
                      if($vo2['gc_id_1']==$vo['gc_id']){
                        
                        $jiage+=$vo2['goods_total'];  
	    ...
	    $model_voucher = Model('voucher');
	        ...
	        $voucher_list[$store_id] = $model_voucher->getCurrentAvailableVoucher($condition,$goods_total,$new_class,$voucher_t_channel);

	    }
            
           if(count($voucher_list[39][0])!=0){
               
               $voucher_list[39][1]=array();
...
//	    }
        ...
        /*edit by peiyu start*/
        public function getStoreAvailableVoucherList1($store_goods_total, $member_id) {
        ...
        public function getVoucher($voucher_id) {
            ...
            $result = $model_voucher->table(voucher)->where(array("voucher_id"=>$voucher_id))->select();
	...
	public function reParseVoucherList($input_voucher_list = array(), $store_goods_total = array(), $member_id) {
	    ...
	    if (empty($input_voucher_list) || !is_array($input_voucher_list)) return array();
            ...
            /*rewrite this contro by peiyu start*/
            
            foreach( $input_voucher_list as $store_id => $voucher_array){
                
               foreach($voucher_array as $k => $voucher){
                   
                   if(is_array($voucher)){
                       
                       /*查询数据库返回*/  
                       
                        $store_voucher_list = $this->getVoucher($voucher['voucher_id']);   

                        $input_voucher_list[$store_id][$k]['voucher_code'] = $store_voucher_list[0]['voucher_code'];

                        $input_voucher_list[$store_id][$k]['voucher_owner_id'] = $store_voucher_list[0]['voucher_owner_id'];             
            ...
            /*rewrite this contro by peiyu end*/
            //print_r( $store_voucher_list);
    ...
    public function getStoreFreightDescList($store_goods_total) {
        ...
        foreach ($store_list as $store_info) {
            ...
            if ($limit_price == 0 || $limit_price > $store_goods_total[$store_info['store_id']]) {
				...
				$store_free_price[$store_info['store_id']]['free_price']=$limit_price;
				$store_free_price[$store_info['store_id']]['desc']=sprintf('满%s免运费',$limit_price);
        ...
        return array($need_calc_sid_array,$cancel_calc_sid_array,$store_free_price);
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
				...
				if($order_list[$key]['shipping_type']==2){$data_order['order_state'] = ORDER_STATE_SELF;}
...
//         if (empty($pd_payment_info)) return;
	...
	public function makePaySn($member_id) {
		...
		return mt_rand(10, 99)
			  ...
			  .substr(sprintf('%010d',time() - 946656000), 0,8)

		      . sprintf('%03d', (float) microtime() * 1000)

		      . sprintf('%03d', (int) 1 % 1000);
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        /*edit by peiyu start修改立即下单查找商品的信息*/
    ...
    public function getGroupbuyInfo(& $goods_info = array()) {
        ...
        $goods_info['goods_price'] = $groupbuy_info['groupbuy_price'];
            ...
            if ($_SESSION['member_id']) {
                ...
                $condition_order['buyer_id'] = $_SESSION['member_id'];

                $condition_order['goods_id'] = $goods_info['goods_id'];



                $order_goods_info = $model_order->getOrderGoodsList($condition_order);

                foreach ($order_goods_info as $ordergoods) {

                    //如果客户下了多个订单，把订单商品表的商品数量加起来
            ...
            $groupbuy_info['upper_limit'] =$groupbuy_info['upper_limit'] - $goods_num;

                if($groupbuy_info['upper_limit']==0){
    ...
    private function _parseMansongRuleDesc($rule_info) {
        ...
        $has_already=Model('order')->getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false);
        if(!$has_already) {
        ...
        return sprintf('我是首单---满%s%s%s',$rule_info['price'],$discount_desc,$goods_desc);
