...
/*	$("#btn_zt").click(function(){	
	...
	$("#btn_kd").click(function(){
		if($("input[name='shipping_type']").val()==1){return false;}
		location.reload();
/*				if($("input[name='shipping_type']").val()==1){return false;}
		$(this).addClass('btn_choose_type');
		$('#btn_zt').removeClass('btn_choose_type');
		$('#store_address').hide();
		$('#cust_address').show();
		$("input[name='shipping_type']").val(1);*/
	});
				...
				htmldata+='<p class="buys-yt-tlt"><!--店铺名称：-->'+v.store_name+'<span data-store_id="'+k+'" class="store-cod-supported" style="display:none;">（该店铺不支持选定收货地址的货到付款）</span></p>';
						...
						$.each(v.goods_list,function(k1,v1){//循环商品列表
							...
							if(j==0){
								...
								htmldata+=	'<div class="buys1-pdlist d_pdrl10">';
							...
							htmldata+='<div class="clearfix d_bd1">'

												+'<a class="img-wp d_wd100" href="'+WapSiteUrl+'/tmpl/product_detail.html?goods_id='+v1.goods_id+'">'
												...
												+'<div class="buys1-pdlcnt">'
													...
													+'<p class="d_dispr"><a class="buys1-pdlc-name" href="'+WapSiteUrl+'/tmpl/product_detail.html?goods_id='+v1.goods_id+'">'+v1.goods_name+'</a></p>'

													+'<p>商品单价：'+'<em>'+'￥'+v1.goods_price+'</em>'+'</p>'

													+'<p>数量：'+'<em>'+v1.goods_num+'</em>'+'</p>'
						...
						htmldata+= '<div class="shop-total d_vr_select d_bd0"><p class="d_black d_cred">运费：<em>￥<span id="store'+k+'" store_id="'+k+'" class="trans_price"></span></em></p>';
						
						if(v.store_free_price != null){
							
							htmldata+= '<p>'+v.store_free_price.desc+',(还差'+(v.store_free_price.free_price - pf(v.store_goods_total))+'元免运费）</p>';
						...
						//1元购 wugangjain 20160921 1元购商品取消代金券使用
						var groupbuy_id = v.goods_list[0].groupbuy_id;
console.log(groupbuy_id);
                        if (v.store_voucher_list[0] && !isEmpty(v.store_voucher_list[0]) && (groupbuy_id == undefined)) {
console.log(groupbuy_id);
                            htmldata+='<p>专项代金券：<select name="voucher" class="d_select_ch" store_id="'+k+'" style="min-width:117px;">';

                            htmldata+='<option value="0">请选择...</option>';

                            $.each(v.store_voucher_list[0],function(k2,v2){

                                htmldata+='<option value="'+v2.voucher_t_id+'|'+k+'|'+v2.voucher_price+'|'+v2.voucher_id+'">'+v2.voucher_price+'元代金券'+'</option>'

                            });

                            htmldata+='</select>:￥-<span id="sv'+k+'" style="display:block;float:right;width:40px;height:30px;line-height:30px;text-align:left">0.00</span></p>';

                        }
						
						if (v.store_voucher_list[1] && !isEmpty(v.store_voucher_list[1]) && (groupbuy_id == undefined)) {
console.log(groupbuy_id);
                            htmldata+='<p>满减代金券：<select name="voucher_1" class="d_select_ch" store_id="'+k+'" style="min-width:117px;">';

                            htmldata+='<option value="0">请选择...</option>';

                            $.each(v.store_voucher_list[1],function(k2,v2){

                                htmldata+='<option value="'+v2.voucher_t_id+'|'+k+'|'+v2.voucher_price+'|'+v2.voucher_id+'">'+v2.voucher_price+'元代金券'+'</option>'

                            });

                            htmldata+='</select>:￥-<span id="sv'+k+'_1" style="display:block;float:right;width:40px;height:30px;line-height:30px;text-align:left">0.00</span></p>';

                        }
						
						if (v.store_voucher_list[2] && !isEmpty(v.store_voucher_list[2]) && (groupbuy_id == undefined)) {
console.log(groupbuy_id);
                            htmldata+='<p>通用代金券：<select name="voucher_2" class="d_select_ch" store_id="'+k+'" style="min-width:117px;">';

                            htmldata+='<option value="0">请选择...</option>';

                            $.each(v.store_voucher_list[2],function(k2,v2){

                                htmldata+='<option value="'+v2.voucher_t_id+'|'+k+'|'+v2.voucher_price+'|'+v2.voucher_id+'">'+v2.voucher_price+'元代金券'+'</option>'

                            });

                            htmldata+='</select>:￥-<span id="sv'+k+'_2" style="display:block;float:right;width:40px;height:30px;line-height:30px;text-align:left">0.00</span></p>';
						...
						if(v.store_mansong_rule_list != null){
							...
							htmldata+='<p class="d_black d_cred">本店合计：<em>￥<span id="st'+k+'" store_price="'+sp_total+'" class="store_total">'+p2f(sp_total)+'</span></em></p>';
...
//			}
				...
				success:function(result){
					...
					if(result.datas.state == 'success'){
						...
						$('input[name=offpay_hash_batch]').val(result.datas.offpay_hash_batch);
                         ...
                         /*eidt by peiyu start 增加剩下两种代金券的效果*/
                         
			$('select[name=voucher]').change(function(){//选择代金券1
				...
				var store_tran = pf($('#store'+store_id).html());
                                ...
                                var voucher_price1 = pf($('#sv'+store_id).html());
                                
                                var voucher_price2 = pf($('#sv'+store_id+'_1').html());
                                
                                var voucher_price3 = pf($('#sv'+store_id+'_2').html());
                                
				store_total = pf(store_total_price) - voucher_price1 - voucher_price2 - voucher_price3 + store_tran;
                ...
                cod.refleshTotals();
                        ...
                        $('select[name=voucher_1]').change(function(){//选择代金券2
				...
				var store_tran = pf($('#store'+store_id).html());
                                ...
                                $('#sv'+store_id+'_1').html(p2f(store_price));
                ...
                cod.refleshTotals();
                        ...
                        $('select[name=voucher_2]').change(function(){//选择代金券3
				...
				var store_tran = pf($('#store'+store_id).html());
                                ...
                                $('#sv'+store_id+'_2').html(p2f(store_price));
                ...
                cod.refleshTotals();
                        ...
                        /*eidt by peiyu stop 增加剩下两种代金券的效果*/
			
			

			//选择到店自提方式
			$("#btn_zt").click(function(){
				if($("input[name='shipping_type']").val()==2){return false;}
				$(this).addClass('btn_choose_type');
				$('#btn_kd').removeClass('btn_choose_type');
				$("input[name='shipping_type']").val(2);
				$('#store_address').show();
				$('#cust_address').hide();
				$('.trans_price').each(function(){
		
					var store_id = $(this).attr('store_id');
			
					var varr = $('select[name=voucher]').val();
					...
					store_total = pf(store_total_price) - store_price;

					$("#store"+store_id).html(p2f(0));
    ...
    $(".buys1-edit-invoice").click(function(){
    	...
    	// 1元购 wugangjian 20160921 1元购活动产品不开发票 START
    	var goods_id = GetQueryString("goods_id");
		var goods_promotion_type;
    	var allcookies = document.cookie.split("member_id=");
    	var member_id = allcookies[1].split(";")[0];

		var wgj_data = {};
		wgj_data.goods_id = goods_id;
		wgj_data.member_id = member_id;

		$.ajax({

			type:'post',

			url:ApiUrl+'/index.php?act=goods&op=wgj_goods_detail&goods_id='+goods_id,
    	...
    	// 1元购 wugangjian 20160921 1元购活动产品不开发票 END
    ...
    $('#buy_step2').click(function(){//提交订单step2
		...
		if(shipping_type == 2){
			...
			var member_truename = $('input[name=member_truename]').val();
...
//        });
        ...
        voucher.push($("select[name=voucher]").val());
        
        voucher.push($("select[name=voucher_1]").val());
        
        voucher.push($("select[name=voucher_2]").val());
		...
		// 1元购 wugangjian 20160922 1元购活动产品不能重复提交订单 START
    	var goods_id = GetQueryString("goods_id");
		var goods_promotion_type;
    	var allcookies = document.cookie.split("member_id=");
    	var member_id = allcookies[1].split(";")[0];
		...
		// 1元购 wugangjian 20160922 1元购活动产品不能重复提交订单 END
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		//1元购活动  wugangjian 20160922 限制每位用户只能购买唯一一款产品 START 
        		if(result.datas.goods_type == 2) {
...
function showPosition(position)
  ...
  x.innerHTML='您距离店铺 ' + GetDistance(position.coords.latitude,position.coords.longitude,28.108900,120.961586) + ' 公里';
  }
//进行经纬度转换为距离的计算
function Rad(d){
