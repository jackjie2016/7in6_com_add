...
<div class="ncc-receipt-info">
  ...
  <table class="ncc-table-style">
    ...
    <!-- 1元购 wugangjian 20160922 隐藏域传wgj_goods_id wgj_member_id START -->
    <!-- <?php var_dump($_SESSION['member_id']); ?> -->
    <!-- $cart_list[0]['goods_id'] -->
    <div class="wgj_goods_id" style="display:none"><?php echo $cart_list[0]['goods_id']; ?></div>
    <div class="wgj_member_id" style="display:none"><?php echo $_SESSION['member_id']; ?></div>
    <!-- 1元购 wugangjian 20160922 隐藏域传wgj_goods_id wgj_member_id END -->
    <tbody>
      <tr>
      ...
      <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
        ...
        <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
            ...
            <!-- wugangjian 20160917 团购标签替换1元购 -->
            <!-- <dd> <span class="groupbuy">团购</span></dd> -->
            <dd> <span class="groupbuy">1元购</span></dd>
            <?php }?>
            <?php if ($cart_info['bl_id'] != '0') {?>
      ...
      <tr>
        <td class="tr" colspan="20"><div class="ncc-store-account">
            ...
            <?php if (!empty($output['store_voucher_list'][$store_id][0]) && is_array($output['store_voucher_list'][$store_id][0])) {?>
            <dl class="voucher">
              <dt>
                <select nctype="voucher" name="voucher_0[<?php echo $store_id;?>]">
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|0.00">选择专项代金券</option>
                  <?php foreach ($output['store_voucher_list'][$store_id][0] as $voucher) {?>
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|<?php echo $voucher['voucher_price'];?>|<?php echo $voucher['voucher_id'];?>"><?php echo $voucher['desc'];?></option>
                  <?php } ?>
                </select>：
              </dt>
            ...
            <?php if (!empty($output['store_voucher_list'][$store_id][1]) && is_array($output['store_voucher_list'][$store_id][1])) {?>
            <dl class="voucher">
              <dt>
                <select nctype="voucher1" name="voucher_1[<?php echo $store_id;?>]">
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|0.00">选择满额代金券</option>
                  <?php foreach ($output['store_voucher_list'][$store_id][1] as $voucher) {?>
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|<?php echo $voucher['voucher_price'];?>|<?php echo $voucher['voucher_id'];?>"><?php echo $voucher['desc'];?></option>
                  <?php } ?>
                </select>：
              </dt>
              <dd><em id="eachStoreVoucher_<?php echo $store_id;?>_1">-0.00</em><?php echo $lang['currency_zh'];?></dd>
            </dl>
            <?php } ?>
            
            <?php if (!empty($output['store_voucher_list'][$store_id][2]) && is_array($output['store_voucher_list'][$store_id][2]) && empty($cart_info['ifgroupbuy'])) {?>
            <dl class="voucher">
              <dt>
                <select nctype="voucher2" name="voucher_2[<?php echo $store_id;?>]">
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|0.00">选择普通代金券</option>
                  <?php foreach ($output['store_voucher_list'][$store_id][2] as $voucher) {?>
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|<?php echo $voucher['voucher_price'];?>|<?php echo $voucher['voucher_id'];?>"><?php echo $voucher['desc'];?></option>
                  <?php } ?>
                </select>：
              </dt>
              <dd><em id="eachStoreVoucher_<?php echo $store_id;?>_2">-0.00</em><?php echo $lang['currency_zh'];?></dd>
            </dl>
            <?php } ?>
