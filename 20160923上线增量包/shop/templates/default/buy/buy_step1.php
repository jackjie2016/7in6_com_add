...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <!--edit by peiyu start  增加shipping_type 隐藏域-->
...
function calcOrder() {
    ...
    $('em[nc_type="eachStoreTotal"]').each(function(){
        ...
        /*edit by peiyu start*/
        if ($('#eachStoreVoucher_'+store_id+'_1').length > 0) {
        	eachTotal += parseFloat($('#eachStoreVoucher_'+store_id+'_1').html());
        }
        if ($('#eachStoreVoucher_'+store_id+"_2").length > 0) {
        	eachTotal += parseFloat($('#eachStoreVoucher_'+store_id+'_2').html());
        }
        /*edit by peiyu stop*/
...
$(function(){
    ...
    /*改动js edit by peiyu start */
     $('select[nctype="voucher1"]').on('change',function(){
        if ($(this).val() == '') {
        	$('#eachStoreVoucher_'+items[1]+'_1').html('-0.00');
        } else {
            var items = $(this).val().split('|');
            $('#eachStoreVoucher_'+items[1]+'_1').html('-'+number_format(items[2],2));
        }
        calcOrder();
    });
    
     $('select[nctype="voucher2"]').on('change',function(){
        if ($(this).val() == '') {
        	$('#eachStoreVoucher_'+items[1]+'_2').html('-0.00');
        } else {
            var items = $(this).val().split('|');
            $('#eachStoreVoucher_'+items[1]+'_2').html('-'+number_format(items[2],2));
        }
        calcOrder();
    });
    /*改动js edit by peiyu end */
