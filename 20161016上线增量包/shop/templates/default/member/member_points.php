...
<div class="wrap">
  ...
  <h4>印币获得规则</h4>
<ul><li>成功注册会员：增加<em><?php echo C('points_reg');?></em>印币；会员每天登录：增加<em><?php echo C('points_login');?></em>印币；评价完成订单：增加<em><?php echo C('points_comments');?></em>印币。</li>
<?php if (C('points_orderrate')) {?><li>购物并付款成功后将获得订单总价<?php printf('%d', 1/C('points_orderrate') * 100);?>%<?php if (C('points_ordermax')) {?>（最高限额不超过<?php echo C('points_ordermax');?>）<?php }?>印币。</li><?php }?>
<li>如订单发生退款、退货等问题时，印币将不予退还。</li></ul>
  </div>
