			...
			$insert_arr['voucher_t_start_date'] = strtotime($_POST['txt_template_startdate']);
	        	...
	        	if ($_POST['txt_template_enddate']){
                    ...
                    if ($isOwnShop)
                        ...
                        /*edit by peiyu start 增加有效期的截止时间*/
                        if ($_POST['txt_template_use_end_date']){

	        		$enddate = strtotime($_POST['txt_template_use_end_date']);
	        		...
	        		$insert_arr['voucher_u_end_date'] = $enddate;

	        	}else {//如果没有添加有效期则默认为套餐的结束时间

                        if ($isOwnShop)

                            $insert_arr['voucher_u_end_date'] = time() + 2592000; // 自营店 默认30天到期

                        else

                            $insert_arr['voucher_u_end_date'] = $quotainfo['quota_endtime'];

	        	}
                        
                        /*edit by peiyu stop 增加有效期的截止时间*/
...
//            Tpl::output('store_class', $store_class);
    ...
    public function templateeditOp(){
        ...
        /*
         * edit by peiyu start 关于修改已经领取代金券的再次修改
         **/
...
//	        }else {
	        	...
	        	if ($_POST['txt_template_enddate']){
                    ...
                    /*edit by peiyu start代金券模版编辑增加有限期结束时间 */
	        		...
	        		$update_arr['voucher_u_end_date'] = $enddate;

	        	}else {//如果没有添加有效期则默认为套餐的结束时间

                        if ($isOwnShop)

                            $update_arr['voucher_u_end_date'] = time() + 2592000; // 自营店 默认30天到期

                        else

                           $update_arr['voucher_u_end_date'] = $quotainfo['quota_endtime'];

	        	}

                    /*edit by peiyu stop代金券模版编辑增加有限期结束时间 */
