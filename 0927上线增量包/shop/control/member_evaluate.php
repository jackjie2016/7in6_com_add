...
class member_evaluateControl extends BaseMemberControl{
    ...
    public function addOp(){
        ...
        /*edit by peiyu start 重组订单*/
       
        $goods_new = array();
        
        foreach($order_goods as $v){
            
            $goods_info = $model_order->table('goods')->where(array("goods_id"=>$v['goods_id']))->select();
            
            $v['goods_commonid'] = $goods_info[0]['goods_commonid'];
            
            $goods_new[] = $v;
        }
        
        $order_goods = $goods_new;
        
        /*edit by peiyu  stop 重组订单*/
        ...
        if (!$_POST){
            ...
            if ($exppoints_rule['exp_comments'] > 0 || $points_comments > 0){
                ...
                if ($points_comments > 0){
                    ...
                    $ruleexplain .= (' “'.$points_comments.'抵币”');
            ...
            foreach ($order_goods as $value){
                ...
                $evaluate_goods_info['geval_ordergoodsid'] = $value['rec_id'];
                 ...
                 $evaluate_goods_info['geval_ordergoodscommonid'] = $value['goods_commonid'];
