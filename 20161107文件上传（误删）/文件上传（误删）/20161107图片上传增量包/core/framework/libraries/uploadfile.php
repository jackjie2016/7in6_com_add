 ...
 * @package    library* shop.easyboxprint.cn 专业团队 提供售后服务
	...
	public function upfile($field){
		...
		if ($this->thumb_width && $this->thumb_height && $this->thumb_ext){
			...
			$thumb_ext 	= explode(',',$this->thumb_ext);
		 	...
		 	if (count($thumb_width) == count($thumb_height) && count($thumb_height) == count($thumb_ext)) $ifresize = true; //如果前台设置缩略图尺寸
...
//				}
		...
		$type = C('upload_service_type');
        	...
        	import('libraries.upload.'.$type);    //引入upyun基类
		...
		if(@move_uploaded_file($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name)){
			...
			if ($ifresize){
				...
				$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
                                ...
                                if(!$upload->save2($resizeImage->relative_dstimg)){
                                        $this->setError($upload->getError());
                                return false;
                                }
                                }
...
//				$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
	...
	private function setFileName(){
        ...
        /*edit by peiyu start 判断是否有汉字*/
            
        $tmp = explode(".",$this->upload_file['name']);
        
        $str = array_pop($tmp);
        
        $tmp = implode(".", $tmp);
        
        if(preg_match("/([\x81-\xfe][\x40-\xfe])/", $tmp, $match)||  strpos( $tmp, '-') !== false ||(strlen($tmp)>15)){
            ...
            $tmp_name = $tmp."-".rand(1,100).rand(1,1000);
        }   
