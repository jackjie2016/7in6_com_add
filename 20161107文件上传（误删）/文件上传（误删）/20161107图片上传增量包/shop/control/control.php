    ...
    protected function getMemberAndGradeInfo($is_return = false){
        ...
        if($_SESSION['member_id']) {
            ...
            if ($member_info){
                ...
                $voucher_model = Model()->table('voucher');
                
                $voucher_1 =  $voucher_model->where("voucher_owner_id=".$_SESSION['member_id'])->select();
                
                $voucher['voucher_count'] = count($voucher_1);
                
                /*edit by peiyu start 请求登陆表*/
                ...
                $is_sign = Model('member')->table('sign')->where($where)->count();
                
                $sign['sign'] = $is_sign;
                
                /* edit by peiyu start判断是否开启签到功能*/
                
                $sign_allow['sign_allow']= C('sign_allow');
                
                /*edit by peiyu stop*/
               
                $member_info = array_merge($member_info,$member_gradeinfo,$voucher,$sign,$sign_allow);
    ...
    private function _getMenuList() {
        $menu_list = array(
			...
			//'member_flea'     => array('name' => '我的闲置', 'url'=>urlShop('member_flea', 'index')),
                        'member_points'     => array('name' => '我的印币', 'url'=>urlShop('member_points', 'index')),
                ...
                'info' => array('name' => '资料管理', 'child' => array(
                        ...
                        'member_image'    => array('name' => '我的图库', 'url'=>urlShop('member_image', 'image')),
