...
class loginControl extends BaseHomeControl {
	...
	public function indexOp(){
        ...
        /*edit by peiyu stop*/
   		//   微信扫码登陆插件start
         if($_GET['code']){
            header("Location: ".SHOP_SITE_URL."/index.php?act=login&op=weixinLogin&code=".$_GET['code']."&state=STATE");
         }
   		//微信扫码登陆插件end      
		...
		if ($result !== false){
			...
			if ($_pic[0] != ''){
				//Tpl::output('lpic',UPLOAD_SITE_URL.'/'.ATTACH_LOGIN.'/'.$_pic[array_rand($_pic)]);
                                Tpl::output('lpic',UPLOAD_SITE_URL.'/'.ATTACH_LOGIN.'/'.$_pic[0]);
			}else{
	...
	*/
	...
	public function usersaveOp() {
         ...
         *        
         *   */
        public function weixinLoginOp() {
         
         $model_member	= Model('member');
 
              
         $aa=file_get_contents("https://api.weixin.qq.com/sns/oauth2/access_token?appid=".WEIXIN_APPID."&secret=".WEIXIN_APPSECRET."&code=".$_GET['code']."&grant_type=authorization_code");
         $obj = json_decode($aa);
         ...
         //验证登陆
         $ee = file_get_contents("https://api.weixin.qq.com/sns/userinfo?access_token=".$access_token."&openid=".WEIXIN_APPID);
         $ee = json_decode($ee);
         ...
         if(!empty($member_info)) {
          ...
          $model_member->createSession($member_info);
          process::clear('login');
          redirect(SHOP_SITE_URL."/index.php?act=member&op=home");  
...
//        $register_info['email'] = $unionid."@weixin.com";
        ...
        $register_info['wecha_id'] = $unionid;
	 ...
	 * @return
	 */
	public function usersave_phoneOp() {
		//重复注册验证
		if (process::islock('reg')){
			showDialog(Language::get('nc_common_op_repeat'));
		}
        ...
        $register_info = array();
        $register_info['username'] = $_POST['user_mobile'];
        $register_info['password'] = $_POST['password'];
        $register_info['password_confirm'] = $_POST['password_confirm'];
        //$register_info['email'] = $_POST['email'];
		$register_info['user_mobile']=$_POST['user_mobile'];
		//添加奖励抵币ID BY shopjl.COM V3
		//$register_info['inviter_id'] = intval($_COOKIE['uid'])/1;
		$register_info['inviter_id'] = intval(base64_decode($_COOKIE['uid']))/1;
    ...
    public function find_pwdOp() {
        ...
        $update	= $model_member->editMember(array('member_mobile'=>$_POST['member_mobile']),array('member_passwd'=>md5($_POST['password'])));
	...
	public function bind_emailOp() {
	   ...
	   if (md5($member_common_info['auth_code']) != $_GET['hash'] || TIMESTAMP - $member_common_info['send_acode_time'] > 24*3600) {
		...
		//新增邮箱绑定
	   $data = array();
	   $data['member_email'] = @base64_decode($_GET['member_email']);
	   $data['member_email_bind'] = $_GET['member_email_bind'];
	   if($member_common_info['member_email_bind']!=$_GET['member_email_bind']){
		   $data['member_email_bind'] = $_GET['member_email_bind'];
	   }
