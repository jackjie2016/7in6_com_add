...
class goodsControl extends BaseGoodsControl {
    public function __construct() {
        ...
        /*edit by peiyu start 增加读取相册的语言文件*/
        Language::read('member_store_album');
        /*edit by peiyu*/
        $this->upGoodsOp();
    }
	public function upGoodsOp(){		$time_now = strtotime(date('Y-m-d H:i:s'));		$condition['goods_state'] = 0;		$condition['goods_selltime'] = array('between',array(100,$time_now));		$update['goods_state'] = 1;		Model()->table('goods_common')->where($condition)->update($update);		$goods_id_array = Model()->table('goods_common')->field('goods_id')->where($condition)->order('goods_id desc')->find();		$condition['goods_id'] = array('in', $goods_id_array);		Model()->table('goods')->where($condition)->update($update);	}
    /**
     * 单个商品信息页
     */
    public function indexOp() {
        ...
        */
        ...
        $wgj_order_id = $model_order_goods->field('order_id')->where($where3)->find();
        $model_orders = Model()->table('order');
        $where2['order_id'] = $wgj_order_id['order_id'];
        ...
        */
        ...
        /**
        *1元购活动 详情页展示已购数量
        *wugangjian 20160917 START
        */
        //获取团购详细信息
        $model_groupbuy = Model('groupbuy');
        $where2['goods_id'] = $goods_id;
        $groupbuy_info = $model_groupbuy->field('*')->where($where2)->find();

        Tpl::output('groupbuy_info',$groupbuy_info);

        /**
        *1元购活动 详情页展示已购数量
        *wugangjian 20160917 END
        */
    ...
    private function _get_comments($goods_id, $type, $page) {
        ...
        $goods = Model("goods");
        $info  = $goods->where("goods_id=$goods_id")->select();
        $condition['geval_ordergoodscommonid'] = $info[0]['goods_commonid'];
    ...
    public function image_uploadOp() {
        ...
        $upload->set('default_dir', ATTACH_IMG.DS.$member_id.DS);
        ...
        if ($result) {
            ...
            $pic = $upload->file_name;

            $pic_thumb =$upload->thumb_image;
        ...
        list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS .ATTACH_IMG . DS . $member_id. DS.$pic);
        ...
        $result = Model('goods')->add($insert_array);
