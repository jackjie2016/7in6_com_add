	...
	public function registerOp(){
		...
		if (chksubmit()){
			...
			if($result) {
	         ...
	         $avatar	= $qquser_info['figureurl_qq_2'];
				$update_info	= array();

				if($avatar) {

				    $update_info['member_avatar'] 	= $avatar;

    				$model_member->editMember(array('member_id'=>$result),$update_info);

    				$user_array['member_avatar'] 	= $avatar;
	...
	public function autologin(){
		...
		if (is_array($member_info) && count($member_info)>0){
			...
			$success_message = Language::get('login_index_login_success');
                        /*edit by peiyu 修改qq登录成功后的跳转*/
			showMessage($success_message,$_COOKIE['ref_url']);
