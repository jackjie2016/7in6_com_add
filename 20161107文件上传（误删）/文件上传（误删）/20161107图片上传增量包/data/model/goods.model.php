...
class goodsModel extends Model{
    ...
    public function getGoodsList($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false, $count = 0) {
		...
		$glist =$this->table('goods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->lock($lock)->select();
		
		foreach($glist as $v){
			$goods_common_info = model()->table('goods_common')->field('goods_name')->where(array('goods_commonid'=>$v['goods_commonid']))->find();
			$v['goods_name']=$goods_common_info['goods_name'];
			if(empty($v['goods_name'])){$v['goods_name']=$field;}
			$gg[]=$v;
		}
	...
	$count = $this->getGoodsOnlineCount($condition,"distinct CONCAT(goods_commonid)");
        ...
        if ($count != 0) {
			...
			//$goods_list = $this->getGoodsCommonOnlineList($condition);
			
        }

		//$goods_list[0]['goods_name']='sssss';
    ...
    public function getGoodsSpecListByCommonId($common_id) {
        ...
        if (empty($spec_list)) {
            ...
            $spec_array = $this->getGoodsList(array('goods_commonid' => $common_id), 'goods_spec,goods_id,store_id,goods_image,color_id,goods_price,goods_marketprice');
    ...
    public function getGoodsDetail($goods_id) {
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $price_array[] =  $value['goods_price'];
            
            $price_array[] = $value['goods_marketprice'];
            ...
            $spec_price = implode('|', $price_array);

            $tpl_spec = array();

            $tpl_spec['sign'] = $spec_sign;
            
             $tpl_spec['price'] = $spec_price;
        ...
        if (!empty($goods_info['groupbuy_info'])) {
            ...
            $goods_info['my_buy_num'] = $goods_num;
        ...
        if (!empty($goods_info['xianshi_info'])) {
            ...
            $goods_info['up_limit'] = $goods_info['xianshi_info']['up_limit'];
            $goods_info['start_time'] = date('Y-m-d H:i:s',$goods_info['xianshi_info']['start_time']);
            $goods_info['end_time'] = date('Y-m-d H:i:s',$goods_info['xianshi_info']['end_time']);
            $goods_info['start_time_int'] = $goods_info['xianshi_info']['start_time'];
            $goods_info['end_time_int'] = $goods_info['xianshi_info']['end_time'];
            $goods_info['now_time_int'] = time();
            $goods_info['shengyu_time'] =time()-$goods_info['start_time_int'];
            ...
            // 我后加的
            $goods_info['xianshi_info']=Model('p_xianshi_goods')->where(array('goods_id'=>$goods_info['goods_id']))->find();

            if(time()<$goods_info['xianshi_info']['end_time']){
                $goods_info['promotion_type'] = 'xianshi';
                ...
                $goods_info['end_time_int'] = $goods_info['xianshi_info']['end_time'];
                $goods_info['now_time_int'] = time();
                $goods_info['shengyu_time'] =$goods_info['start_time_int']-time();
