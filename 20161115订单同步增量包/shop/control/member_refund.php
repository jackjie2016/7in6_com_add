 ...
 *
 *
 **by 样样抵商城V3 shop.easyboxprint.cn 运营版*/
...
class member_refundControl extends BaseMemberControl {
			...
			if ($state) {
			    $model_refund->editOrderLock($order_id);
                            ...
                            if($obj->code==1){
                                ...
                                $path = $dir.$name;
                                $content = date('y-m-d h:i:s',time()).'订单编号为'.$obj->sp_orderNum."退款状态同步到工单失败 \r\n";
                                w_log($path,$content);
                            }else if($obj->code==0){
                            /* edit by peiyu stop*/
	 ...
	 *
	 */
	public function add_refund_wapOp(){
		...
		if ($refund['refund_id'] > 0 || $order['order_state'] != $order_paid || $payment_code == 'offline') {/* 检查订单状态,防止页面刷新不及时造成数据错误 */
			...
			showDialog('您申请的退款正在审核中','','error');

		}
		
		if (chksubmit() || isset($_POST['order_refund_amount'])) {
			...
			if ($state) {
				...
				/*echo "<script>alert('申请成功!')</script>";*/
                                ...
                                /* edit by peiyu stop*/
				echo "<script>window.location.href ='http://root.7in6.com/wap/tmpl/member/order_list.html?order_state=20'</script>";
	    ...
	    echo "<script>window.location.href ='http://yangyangdi.com/wap/tmpl/member/order_list.html?order_state=20'</script>";
