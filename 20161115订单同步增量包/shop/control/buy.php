...
class buyControl extends BaseBuyControl {
    ...
    */
    ...
    /* 
    *1元购 wugangjian 20160921 限制每位用户只能购买唯一一款产品 START 
    *新建function wgj_goods_detail2Op
    */
    public function wgj_goods_detail2Op() {
    ...
    /* 
    *1元购 wugangjian 20160921 限制每位用户只能购买唯一一款产品 END 
    *新建function wgj_goods_detail2Op
    */
   ...
   /*   if($_SESSION['member_id']==2805){
             $model_order = Model('order');
                //$where['pay_sn'] = $_GET['pay_sn'];
		        $where['pay_sn'] = '8005325715221001';
                $order_info = Model()->table('order')->where($where)->select();
        ...
        *promotion_type
        */
        $sss = $result['store_cart_list']['39']['0']['groupbuy_info'];
        ...
        Tpl::output('promotion_type'  ,  $promotion_type);
    ...
    public function wxpayOp() {
        ...
        //$url1 = $notify->GetPrePayUrl($pay_sn);
...
//         $input->SetAttach($result['data']['order_type']);

		//加入自定义数据
 
        
       //线上代码
        //$trade_no=$order_list['0']['order_sn'];
		$trade_no=$pay_sn;//微信扫码 2016-11-15
       //先下使用
        //$trade_no=WxPayConfig::MCHID . date("YmdHis");
        $input->SetAttach('real_order');
        ...
        $input->SetGoods_tag($pay_sn);
		 $input->SetNotify_url(SHOP_SITE_URL."/api/payment/wxpay/notify_url_new.php"); //微信扫码 2016-11-15 
/* if($_SESSION['member_id']==2922){ //微信扫码 2016-11-15 
        $input->SetNotify_url(SHOP_SITE_URL."/api/payment/wxpay/notify_url_new.php");
	...
	$input->SetNotify_url(SHOP_SITE_URL."/api/payment/wxpay/notify_url.php");
        ...
        //本地有
        Tpl::output('trade_no', $trade_no);
       
        Tpl::output('total_fee', $pay_amount);

        Tpl::output('resource_site_url',RESOURCE_SITE_URL);

     	Tpl::output('shop_site_url', SHOP_SITE_URL);

        Tpl::output('code_url', $url2);

        Tpl::showpage('wxpay', 'null_layout');

    }
    public function wxpaypdrOp() {
        ...
        $result = $logic_payment->getPdOrderInfo($pay_sn, $_SESSION['member_id']);
        ...
        $pay_info = $model_order->getPdRechargeInfo(array('pdr_sn' => $pay_sn, 'pdr_member_id' => $_SESSION['member_id']));
        ...
        $pay_amount_online = $pay_info['pdr_amount'];

        $pay_amount_offline = $pay_info['pdr_amount'];

        //订单总支付金额(不包含货到付款)

        $pay_amount =  $pay_info['pdr_amount'] ;
...
//         $input->SetAttach($result['data']['order_type']);
        ...
        Tpl::showpage('wxpaypdr', 'null_layout');
