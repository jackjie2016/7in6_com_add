 ...
 * @return string
 */
function getUpimages($member_id,$Path,$ext){
        //字符串转化
        $type = substr($Path, strrpos($Path, '.'));
        $name = substr($Path, 0, strrpos($Path, '.'));
        $Path = $name.'_'.$ext.$type;
	//判断图片服务器
        if(C('upload_service_enabled') == 1){
                return C('upload_service_domain') .DS. ATTACH_IMG .DS.$member_id.DS.$Path;
	}else{
            if (file_exists(BASE_UPLOAD_PATH.DS. ATTACH_IMG .DS.$member_id.DS.$Path)){    	 
            	return BASE_UPLOAD_PATH.DS. ATTACH_IMG .DS.$member_id.DS.$Path;
            } else {
                return UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.C('default_user_portrait');
            }
       }
...
function getMbSpecialImageUrl($image_name = '') {
   ...
   if(count($name_array) == 2) {
	...
	if(C('upload_service_enabled') == 1){
			  return C('upload_service_domain') . $image_path;
		}
    if(is_file(BASE_UPLOAD_PATH . $image_path)) {
