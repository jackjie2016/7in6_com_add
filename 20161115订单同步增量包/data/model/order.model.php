 ...
 * by shopjl 网店交流z3  www.shopjl.com 开发
...
class orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
		...
		$member_info = $this->table('member')->where(array('member_id'=>$order_info['buyer_id']))->find();
		
		$order_info['buyer_true_name']=$member_info['member_truename'];
		
		$order_info['buyer_true_mobile']=$member_info['member_mobile'];
        ...
        if (in_array('order_common',$extend)) {
            ...
            /*edit by peiyu start 增加回传的值*/
           // $order_info['extend_order_common']['order_message'] = unserialize($order_info['extend_order_common']['order_message']);
            /*edit by peiyu stop*/
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        foreach ($list as $order) {
			...
			$list_common = $this->table('order_common')->where(array('order_id'=>$order['order_id']))->select();
			$order['dlyo_pickup_code'] = $list_common[0]['dlyo_pickup_code'] ; //自提码
            ...
            if ($order['order_state'] > 10) {
                ...
                $sql = "SELECT buyuid  FROM `sykjshop_user_money_change` WHERE order_sn = '" . $order['order_sn'] . "' LIMIT 1";
                ...
                if (!$res) {
                    ...
                    $sql = "SELECT * FROM `sykjshop_order_goods` WHERE order_id = " . $order['order_id'];
                    ...
                    //重新拼接数组
                    if(!empty($list2)){// edit by peiyu 
                    foreach ($list2 as $val) {

                        $sql = "SELECT  *  FROM `sykjshop_goods` WHERE goods_id = " . $val['goods_id'];

                        $res = $this->query($sql);

                        //查出商品返利金额

                        $rank1 = $res[0]['rank1'];    //1级返佣比例

                        $rank2 = $res[0]['rank2'];    //2级返佣比例

                        $rank3 = $res[0]['rank3'];    //3级返佣比例

                        //3级返利开始

                        //查找我的上级 

                        $sql = "SELECT parent_uid  FROM `sykjshop_user_tuijian` WHERE uid ='" . $order['buyer_id'] . "'  LIMIT 1";

                        $res = $this->query($sql);

                        $rank1_user = $res[0]['parent_uid'];
                        ...
                        if ($rank1_user) {
                            ...
                            $count = ($val['goods_price'] * $proportion) * $val['goods_num'];

                            $money_change = array();

                            $money_change['changedesc'] = "购买商品返佣金";

                            $money_change['order_sn'] = $order['order_sn'];

                            $money_change['money'] = $count;

                            $money_change['uid'] = $rank1_user;

                            $money_change['buyuid'] = $order['buyer_id'];

                            $money_change['type'] = "system";

                            $money_change['time'] = time();

                            $money_change['thismonth'] = date("Y-m-d", time());

                            $money_change['thism'] = date("Y-m", time());
                            ...
                            if ($count > 0) {
                                ...
                                $this->table('user_money_change')->insert($money_change);

                                //账户加钱

                                $sql = "UPDATE  `sykjshop_member` SET `available_predeposit` =  `available_predeposit` + " . $count . "  WHERE  `member_id` = " . $rank1_user;
                            ...
                            $sql = "SELECT parent_uid  FROM `sykjshop_user_tuijian` WHERE uid ='" . $rank1_user . "'  LIMIT 1";

                            $res = $this->query($sql);

                            $rank2_user = $res[0]['parent_uid'];
                            ...
                            if ($rank2_user) {
                                ...
                                if ($count > 0) {
                                    ...
                                    $sql = "UPDATE  `sykjshop_member` SET `available_predeposit` =  `available_predeposit` +" . $count . "  WHERE  `member_id` = " . $rank2_user;
                                ...
                                $sql = "SELECT parent_uid  FROM `sykjshop_user_tuijian` WHERE uid ='" . $rank2_user . "'  LIMIT 1";

                                $res = $this->query($sql);

                                $rank3_user = $res[0]['parent_uid'];
                                ...
                                if ($rank3_user) {
                                    ...
                                    if ($count > 0) {
                                        ...
                                        $sql = "UPDATE  `sykjshop_member` SET `available_predeposit` =  `available_predeposit` + " . $count . "  WHERE  `member_id` = " . $rank3_user;
        ...
        if (in_array('order_common',$extend)) {
            ...
            foreach ($order_common_list as $value) {
				...
				//$order_list[$value['order_id']]['extend_order_common']['dlyo_pickup_code'] = $value['dlyo_pickup_code'];
    ...
    /*edit by peiyu start 修改获取订单的信息*/
    public function getOrderList1($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(order_common,order_goods), $master = false){
