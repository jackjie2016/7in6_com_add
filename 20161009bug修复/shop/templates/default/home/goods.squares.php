  ...
  <ul class="list_pic">
    ...
    <li class="item">
      <div class="goods-content" nctype_goods=" <?php echo $value['goods_id'];?>" nctype_store="<?php echo $value['store_id'];?>">
        ...
        <!-- wugangjian 20160917 团购商品标签更换为1元购标签 -->
        <!-- <div class="goods-promotion"><span>团购商品</span></div> -->
        <div class="goods-promotion"><span>1元抢购</span></div>
        ...
        <div class="goods-info">
          ...
          <div class="sell-stat">
            <ul>
              <li><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $value['goods_id']));?>#ncGoodsRate" target="_blank" class="status">
                   <!--edit by peiyu start 修改展示的销量-->
                   <?php 
                        $goods1 = Model('goods');
                        
                        $goods_arr = $goods1->table('goods')->where(array('goods_commonid' => $value['goods_commonid']))->select();
                        
                        $goods_salenum_all = 0;
                        
                        foreach ( $goods_arr as $v1){
                            
                            $goods_salenum_all += $v1['goods_salenum'];
                            
                        }
                      
                       echo $goods_salenum_all;
...
/*============= 15/6/27 ==============*/
/* 新增商品成列的边框判断 */
  $('.squares ul.list_pic').find('li').each(
    function(){
      var dH = $(this).find('.goods-sub').height();
      /*var bH = $(this).find('.goods-content').height();*/
      if(dH==0){
          $(this).find('.goods-content').css({height:"380px"});
        }else{
          $(this).find('.goods-content').css({height:"370px"});
        };
      $(this).hover(function(){
        if(dH==0){
          $(this).find('.goods-content').css({height:"368px"});
        }else{
          $(this).find('.goods-content').css({height:"441px"});
        };
      },function(){
        if(dH==0){
          $(this).find('.goods-content').css({height:"380px"});
