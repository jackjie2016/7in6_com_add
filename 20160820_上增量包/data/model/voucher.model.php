...
class voucherModel extends Model {
	...
	public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0,$new_class,$voucher_t_channel) {
          // if($new_class!=''){   
           
            /*
            * 第一步：查询适合特定种类的抵扣券()，只支持使用一张
            * @专项代金券的特征是 voucher_t_kind=1 voucher_limit(随便，当为0的时候相当于专项代金券)  voucher_t_sc_id不为0（为0的时候代表全平台适用,当满减就是第二种，无限制的时候就是第三种）
           */
           $voucher_list = array();
	   ...
	   $condition['voucher_state'] = 1;
           ...
           $condition['voucher_t_channel']=array(array('eq',$voucher_t_channel),array('eq',2),'or');
           
           foreach($new_class as $v){
               
               if($v['jiage']>=0){
                   
                   $condition['voucher_t_sc_id']=array('eq',$v['gc_id']);
                   
                   $condition['voucher_limit']=array(array('eq',$v['jiage']),array('lt',$v['jiage']),'or');
                   
                   $condition['voucher_t_kind']=array('eq',1);
                   
                   if(count($this->table('voucher')->where($condition)->select())!=0){
           ...
           /*
            * 第二步：查询适合全部商品的代金券（抵扣券） 此特征为voucher_limit>0  voucher_t_sc_id=0  voucher_t_kind=1
            */
           
           $voucher_list1=array();
           
           $condition['voucher_limit']=array('lt',$goods_total);
           
           $condition['voucher_t_sc_id']=array('eq',0);
           
           $condition['voucher_t_kind']=array('eq',1);
          
	   $voucher_list1 = $this->table('voucher')->where($condition)->select();
           
           foreach ($voucher_list1 as $key => $voucher) {

	           $voucher_list1[$key]['desc'] = sprintf('面额%s元 有效期至 %s',$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));

	   }
           
           
           /*第三步：查询通用券 此特征为voucher_limit>0   voucher_t_sc_id=0  voucher_t_kind=0
            *            
            *  */
           
           $voucher_list2 = array();
           
           $condition['voucher_t_kind']=array('eq',0);
          
	   $voucher_list2 = $this->table('voucher')->where($condition)->select();
           
           foreach ($voucher_list2 as $key => $voucher) {

	           $voucher_list2[$key]['desc'] = sprintf('面额%s元 有效期至 %s',$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));

	   }
           
           /*将三个数组合并返回*/
           
           $voucher_list_all = array();
           
           $voucher_list_all[]=$voucher_list;
           
           $voucher_list_all[]=$voucher_list1;
           
           $voucher_list_all[]=$voucher_list2;
           
           /*重新将新数组赋给vouch_list*/

	   $voucher_list = $voucher_list_all;
...
//           }
	...
	//取得有效代金券数量
	 ...
	 public function getAvailableVoucherCount($member_id){
	...
	public function getCanChangeTemplateInfo($vid,$member_id,$store_id = 0){
	    ...
	    if (intval($member_info['member_points']) < intval($template_info['voucher_t_points'])){
	        ...
	        return array('state'=>false,'msg'=>'您的抵币不足，暂时不能兑换该代金券');
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null) {
		...
		$field = 'voucher_id,voucher_code,voucher.voucher_t_sc_id,voucher.voucher_t_kind,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_order_id,voucher_store_id,store_name,store_id,store_domain,voucher_t_customimg';
