...
class orderLogic {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $if_quque = true) {
        try {
            ...
            /*变更代金券的状态 start*/
            $order_common = $model_order->table('order_common')->where(array('order_id'=>$order_id))->select();
            $code = $order_common[0]['voucher_code'];
            $code = explode("|", $code);
            foreach($code as $v){
                $data = array(
                    'voucher_state'=>1,
                    );
                $model_order->table('voucher')->where(array('voucher_code'=>$v))->update($data);
            }
            /*变更代金券的状态 stop*/
