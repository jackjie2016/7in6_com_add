 ...
 **by 网店技术交流中心 shop.easyboxprint.cn*/
...
class orderControl extends SystemControl{
	...
	public function indexOp(){
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $order_info = $model_order->getOrderInfo(array('order_id'=>$order_info[order_id]),array('order_goods','order_common','store'));
            
            /*edit by peiyu start 增加商品名称 收货人 收货地址  联系号码*/
            
            $order_list[$order_id]['goods_name']  = $order_info['extend_order_goods'][0]['goods_name'];
	...
	$lang	= Language::getLangContent();
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				$data = $model_order->getOrderList1($condition,'','*','order_id desc',self::EXPORT_SIZE);
			...
			$data = $model_order->getOrderList1($condition,'','*','order_id desc',"{$limit1},{$limit2}");                          
	...
	private function createExcel($data = array()){
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_xtimd'));
                ...
                /*edit by peiyu start 增加的到处的字段 分别为商品名称，收货人收获地址，联系号码*/
                
                $excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_goodsname'));
                
                $excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_receivename'));

		$excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_address'));

		$excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_tel'));
                
                $excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_phone'));
                
                $excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_receiveinfo'));
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_state'));
                ...
                $excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_od_shipping'));
		...
		foreach ((array)$data as $k=>$v){
			...
			$tmp[] = array('data'=>date('Y-m-d H:i:s',$v['add_time']));
                        ...
                        /*edit by peiyu start 增加的数据*/
                        
                        $tmp[] = array('data'=>$v['extend_order_goods'][0]['goods_name']);
                        
                        $tmp[] = array('data'=>$v['extend_order_common']['reciver_name']);
                        
                        $tmp[] = array('data'=>$v['extend_order_common']['reciver_info']['address']);
                        
                        $tmp[] = array('data'=>$v['extend_order_common']['reciver_info']['tel_phone']);
