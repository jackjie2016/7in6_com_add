...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_images_array'] = $_POST['image_path_array'];
            ...
            $common_array['goods_min_order']     = intval($_POST['g_min_order']);
            $common_array['goods_per_plus']     = intval($_POST['g_per_plus']);
            $common_array['goods_attr']         = serialize($_POST['attr']);
            $common_array['goods_body']         = $_POST['g_body'];
            
            //微信三级返利插件start
            $common_array['rank1']        = floatval($_POST['rank1']);
            $common_array['rank2']        = floatval($_POST['rank2']);
            $common_array['rank3']        = floatval($_POST['rank3']);
             //微信三级返利插件end   
            ...
            $common_array['goods_selltime']     = strtotime($_POST['starttime']) + intval($_POST['starttime_H'])*3600 + intval($_POST['starttime_i'])*60;						$common_array['purchase_quantity']	 = $_POST['quota'];
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['goods_min_order']     = intval($_POST['g_min_order']);
                        $goods['goods_per_plus']     = intval($_POST['g_per_plus']);
                        $goods['goods_image']       = $common_array['goods_image'];
						$goods['goods_images_array']       = $common_array['goods_images_array'];
                       ...
                       //微信三级返利插件start
                       $goods['rank1']        = $common_array['rank1'];
                       $goods['rank2']        =$common_array['rank2'];
                       $goods['rank3']        =$common_array['rank3'];					   					   $goods['purchase_quantity']        =$common_array['purchase_quantity'];
                        //微信三级返利插件end
                       $goods['goods_virtual_salenum'] = rand (0,10);
			...
			$save_file = BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$_SESSION['store_id'].DS.$goods_id.'.png';
			//上传图片
			$upload->save2($save_file);
                    ...
                    $goods['goods_min_order']     = $common_array['goods_min_order'];
                    $goods['goods_per_plus']     = $common_array['goods_per_plus']; 
    ...
    public function add_step_threeOp() {
        ...
        $model_goods = Model('goods');
        $img_array = $model_goods->getGoodsList(array('goods_commonid' => $common_id), 'color_id,goods_image,goods_images_array', 'color_id');
        $goods_images_array = explode(",",$img_array[0]['goods_images_array']);
        $k=count($goods_images_array);
        ...
        if (!empty($img_array)) {
			...
			foreach ($img_array as $val) {
				...
				//$image_array[$val['color_id']][$i]['goods_image'] = $goods_images_array[$i];
                $image_array[$val['color_id']][0]['is_default'] = 1;
                $colorid_array[] = $val['color_id'];
				for($j=1;$j<$k;$j++){
						$image_array[$val['color_id']][$j]['goods_image'] = $goods_images_array[$j];
				}
            }
...
/*			for($j=$i;$j<$k;$j++){
     ...
     * AJAX从淘宝链接里抓取商品价格和图片
     */
	public function check_g_serialOp(){
		$g_serial = $_POST['g_serial'];
		$store_id = $_SESSION ['store_id'];
		$datas = array();
		$goods_list = model()->table('goods_common')->where(array('goods_serial'=>$g_serial,'store_id'=>$store_id))->select();
		$datas['count'] = count($goods_list);
     ...
     * AJAX从淘宝链接里抓取商品价格和图片
     */
	public function get_info_taobaoOp(){
		
		$datas = array();
		
		$taobao_url = $_POST['taobao_url'];
		$gongqiu_url = $_POST['gongqiu_url'];
		$get_image = $_POST['get_image'];
		
		$datas['url'] = array('taobao_url'=>$taobao_url,'gongqiu_url'=>$gongqiu_url);
		
		if(!empty($gongqiu_url)){
			$str=false;
			while(!$str){
				$str = $this->http_request_json($gongqiu_url);
				$str = mb_convert_encoding($str, 'UTF-8', 'UTF-8,GBK,GB2312,BIG5' );
				$k++;
			} 
			$str_begin='<div class="text-detail">';
			$begin = strpos ($str,$str_begin);
			$str_end='<!--Baidu AD -->';
			if(empty($begin)){
				$str_begin='<div class="content fl-clr">';
				$begin = strpos ($str,$str_begin);
				$str_end='<!--百度原生广告 begin-->';
			}
			$end = strpos ($str,$str_end);
			$str = substr($str,$begin,$end-$begin);
			$datas['gongqiu_detail'] = $str;
		}
		...
		if(!empty($taobao_url)){		
				...
				set_time_limit(0);
				ini_set('user_agent','Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-CN; rv:1.9.2.12) Gecko/20101026 Firefox/3.6.12');
				ini_set('default_socket_timeout',    120);
				
				$k=0; 
				$str=false;
				while(!$str){
					$str = $this->http_request_json($taobao_url);
					$str = mb_convert_encoding($str, 'UTF-8', 'UTF-8,GBK,GB2312,BIG5' );
					$k++;
				} 
				...
				preg_match_all('/(<em)(\s){1,}(class="tb-rmb-num").*em>/', $str, $m3);
				
				$taobao_price = str_replace(array('<em class="tb-rmb-num">','</em>'),'',$m3[0][0]);
				
				if(empty($taobao_price)){
					preg_match_all('/(<span)(\s){1,}(class="tm-price").*span>/', $str, $m3);
					$taobao_price = str_replace(array('<span class="tm-price">','</span>'),'',$m3[0][0]);
				}
				$datas['taobao_price'] = $taobao_price;
				
				//获取标题
				
				preg_match_all('/<h3\s.*?>/', $str, $t1);
				$tt2=preg_match_all('/(\w+)\s*=\s*(?:(?:(["\'])(.*?)(?=\2))|([^\/\s]*))/', $t1[0][0], $t2);
					for($j=0; $j<$tt2; $j++) {	//将匹配完的结果进行结构重组
						$t2[1][$j] = !empty($t2[4][$j]) ? $t2[4][$j] : $t2[3][$j];
					}
				
				$datas['taobao_title'] = $t2[1][1];
				
				if(empty($t2[1][1])){
						preg_match_all('/<h[1-9] data-spm=\"1000983\">[\s]*(.*)[\s]*<\/h[1-9]>/', $str, $t1);
						$datas['taobao_title'] = $t1[1][0];
				}
				
				//获取卖点  还未完成//
		
				preg_match_all('/<p[\s\S]{0,300}p>/i', $str, $hot1);
				
				$taobao_title2 = trim(str_replace(array(' ','<p>','</p>','\r','\n',' '),'',$hot1[0][2]));
				...
				$image_url = array();
				$datas['get_image_error'] = 0;
				$c1 = preg_match_all('/<img\s.*?>/', $str, $m1);	//先取出所有img标签文本
				$image_name_str = "";
				//$c1 = 6;
				$result = array();
				$ii=0;
				for($i=1; $i<$c1; $i++) {	//对所有的img标签进行取属性
					$c2 = preg_match_all('/(\w+)\s*=\s*(?:(?:(["\'])(.*?)(?=\2))|([^\/\s]*))/', $m1[0][$i], $m2);	//匹配出所有的属性
					for($j=0; $j<$c2; $j++) {	//将匹配完的结果进行结构重组
						$list[$i][$m2[1][$j]] = !empty($m2[4][$j]) ? $m2[4][$j] : $m2[3][$j];
					}
					
					$image_url[$i]="https:" . $list[$i]['src'];
					
					if(stripos($image_url[$i],'_50x50')<=0 and stripos($image_url[$i],'_60x60')<=0){continue;}
					
					//$image_url[$i] = substr($image_url[$i],0,stripos($image_url[$i],"jpg_")+3);
					
					if($image_url[$i] = substr($image_url[$i],0,stripos($image_url[$i],"jpg_")+3)){
							// 上传图片
							$file = $image_url[$i];
							$file_name = basename($image_url[$i]);
							
							if($file_name == "T1FeW3XXNfXXXXXXXX-36-36.gif"){continue;}
							//后缀
							$tmp_ext = explode(".", $file_name);
							$file_type = $tmp_ext[count($tmp_ext) - 1];
							
							$sourimgname = $_SESSION ['store_id'] . "_" . sprintf('%010d',time() - 946656000)
								. sprintf('%03d', microtime() * 1000)
								. sprintf('%04d', mt_rand(0,9999));
							$path = BASE_UPLOAD_PATH.DS . ATTACH_GOODS . DS . $_SESSION ['store_id'];
							
							if(!file_exists($path)){if(mkdir($path,0777)){$datas['create_path']='1';}}else{$datas['path_exist'] = 1;}
							
							$upload_path = $path . DS . $file_name;
							
							$upload_path = $path . DS . $sourimgname .'.'. $file_type ;
							$fz60	= $path.DS.$sourimgname.'_60.'. $file_type;
							$fz240	= $path.DS.$sourimgname.'_240.'. $file_type;
							$fz360  = $path.DS.$sourimgname.'_360.'. $file_type;
							$fz480  = $path.DS.$sourimgname.'_480.'. $file_type;
							$fz1280 = $path.DS.$sourimgname.'_1280.'. $file_type;
		
							$datas['filetype']=$file_type;
							if(copy($file,$upload_path)){
								if(copy($upload_path,$fz60)){
									//更改图片大小
									$this->resizeimage($fz60,60,60, $fz60);
								}
								if(copy($upload_path,$fz240)){
									//更改图片大小
									$this->resizeimage($fz240,240,240, $fz240);
									
								}
								if(copy($upload_path,$fz360)){
									//更改图片大小
									$this->resizeimage($fz360,360,360, $fz360);
								}
								if(copy($upload_path,$fz480)){
									//更改图片大小
									$this->resizeimage($fz480,480,480, $fz480);
								}
								if(copy($upload_path,$fz1280)){
									//更改图片大小
									$this->resizeimage($fz1280,1280,1280, $fz1280);
								}
								
							}else{
								$datas['get_image_error'] = 1;
							}
							$ii++;
							$datas['image'][$ii] = UPLOAD_SITE_URL. DS . ATTACH_GOODS . DS . $_SESSION ['store_id'] . DS.$sourimgname.'_240.'. $file_type;
							$datas['image_name'][$ii] = $sourimgname.'_240.'. $file_type;
							
							$image_name_str = $image_name_str.','.$datas['image_name'][$ii];
		/*					$result[$i] = true;
							$result[$i] = copy($file,$upload_path);
							if(!$result[$i]){$datas['get_image_error'] = '获取图片失败';}
							$datas['file_name'] = $file_name;
							$datas['upload_path'] = $upload_path;*/
							//$datas['upload_path'] = $upload_path;
			...
			//$datas['image_array'] = $image_url;
			$datas['image_name_str']=$image_name_str;
		}
 ...
 */
	private function resizeimage($srcfile,$ratew='',$rateh='', $filename = "" ){
		...
		////echo '源文件的高度'.$srch.'<br />';
		//目的图片的宽度和高度
/*		if($ratew>$srcw && $rateh>$srch){*/
			$dstw=$ratew;
			$dsth=$ratew*$srch/$srcw;
/*		}else{
			$dstw=$srcw;
			$dsth=$srch;
		}*/
