 ...
 * @package    function* shop.easyboxprint.cn 专业团队 提供售后服务
...
function thumb($goods = array(), $type = ''){
 //判断是否开启了图片服务器
    if(C('upload_service_enabled') == 1){
        ...
        //$url = $thumb_host . '/' . $store_id . '/' . $goods['goods_image'];
		$url =  $thumb_host . '/' . $store_id . '/' . ($type == '' ? $goods['goods_image'] : str_ireplace('.', '_' . $type . '.', $goods['goods_image']));
    }else{
 ...
 */
function cthumb2($file, $type = '', $store_id = false) {
    $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
    if (!in_array($type, $type_array)) {
        $type = '240';
    }
    ...
    if ( !file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) )) {
		if($type=480){$thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_GOODS;return $thumb_host . '/' . $store_id . '/' . $file;}else{
...
function cthumb($file, $type = '', $store_id = false) {
 //判断是否开启了图片服务器
	...
	if($type == '' || $type==1){$str = $file;}else{ $str =str_ireplace('.', '_' . $type . '.', $file);}
	
      $url =$thumb_host . '/' . $store_id . '/' . $str;
...
function goodsQRCode($goods_info) {
 ...
 if(C('upload_service_enabled') == 1){
            $thumb_host = C('upload_service_domain') ;
            return $thumb_host .DS.ATTACH_STORE.DS.$goods_info['store_id'].DS.$goods_info['goods_id'].'.png';
        }else{ 
