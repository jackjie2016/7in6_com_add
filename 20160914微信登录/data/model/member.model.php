...
class memberModel extends Model {
    ...
    public function getTuijianCount1($member_id) {

        $sql = "SELECT COUNT(*) as total  FROM `sykjshop_user_tuijian` WHERE parent_uid = '" . $member_id . "' LIMIT 1";
    ...
    public function getTuijianCount2($member_id) {

        $sql = "SELECT COUNT(tb1.id) as total FROM `sykjshop_user_tuijian` AS tb1";

        $sql .= " LEFT JOIN `sykjshop_user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";

        $sql .= " WHERE tb1.parent_uid='$member_id' AND tb2.uid IS NOT NULL  LIMIT 1";
    ...
    public function getTuijianCount3($member_id) {

        $sql = "SELECT COUNT(tb1.id)  as total  FROM `sykjshop_user_tuijian` AS tb1";

        $sql .= " LEFT JOIN `sykjshop_user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";

        $sql .= " LEFT JOIN `sykjshop_user_tuijian` AS tb3 ON tb3.parent_uid = tb2.uid ";

        $sql .= " WHERE tb1.parent_uid='$member_id' AND tb3.uid IS NOT NULL  LIMIT 1";
    ...
    public function getInvitePeople($member_id) {

        $sql = "SELECT `parent_uid` FROM `sykjshop_user_tuijian` WHERE `uid` =" . $member_id;

        $res = $this->query($sql);



        $sql = "SELECT `member_name`,`nickname` FROM `sykjshop_member` where `member_id` = " . $res[0]['parent_uid'];
    ...
    public function getIncomeYesterday($member_id) {

        $sql = "SELECT SUM(tb1.money) as total  FROM `sykjshop_user_money_change` AS tb1 LEFT JOIN `sykjshop_order` AS tb2 ON tb2.order_sn = tb1.order_sn WHERE tb1.uid = '" . $member_id . "' AND tb2.order_state > 10 AND tb1.money > 0  and TO_DAYS( NOW( ) ) - TO_DAYS(tb2.payment_time) <= 1   LIMIT 1";
    ...
    public function getIncomeCount($member_id) {

        $sql = "SELECT SUM(tb1.money) as total  FROM `sykjshop_user_money_change` AS tb1 LEFT JOIN `sykjshop_order` AS tb2 ON tb2.order_sn = tb1.order_sn WHERE tb1.uid = '" . $member_id . "' AND tb2.order_state > 10 AND tb1.money > 0   LIMIT 1";
    ...
    public function getMemberMoneyChangeByID($member_id) {

        // 	$getUserMoneyLogSql = "select *,count(*) from sykjshop_user_money_change where uid=".$member_id." GROUP BY thismonth DESC";

        $getUserMoneyLogSql = "select * from sykjshop_user_money_change where uid=" . $member_id . " ORDER BY thismonth DESC";

        $moneychangelog = array();

        $moneychangelog = $this->query($getUserMoneyLogSql);
    ...
    public function getMemberMoneyChangeInfo($member_id, $time) {

        $getMemberMoneySql = "select * from sykjshop_user_money_change where uid=" . $member_id . " and thismonth='" . $time . "'";

        return $this->query($getMemberMoneySql);
    ...
    public function getMemberInfoByID($member_id, $fields = '*') {
        ...
        $member_info['zcount1'] = $this->getTuijianCount1($member_id);

        $member_info['zcount2'] = $this->getTuijianCount2($member_id);

        $member_info['zcount3'] = $this->getTuijianCount3($member_id);



        //邀请人

        $member_info['invite_people'] = $this->getInvitePeople($member_id);



        //昨日收益

        $member_info['income_yesterday'] = $this->getIncomeYesterday($member_id);



        //总收益

        $member_info['income_count'] = $this->getIncomeCount($member_id);
    ...
    /* 微信扫码登陆插件start、微信登陆插件start */

    public function updateWeixinMember($condition, $data) {
    ...
    public function createSession($member_info = array(), $reg = false) {
        ...
        if (trim($member_info['wecha_id'])) {

            $_SESSION['weixin_openid'] = $member_info['wecha_id'];
    ...
    public function register($register_info,$type="normal") {
        ...
        $member_info['wecha_id'] = $register_info['wecha_id'];
 
        $member_info['weixin_unionid'] = $register_info['unionid'];
		
		 $weixin_info = array();
           $weixin_info['unionid'] = $register_info['unionid'];
          $weixin_info['nickname'] = $register_info['nickname'];
          $weixin_info['openid'] = $register_info['openid'];
          $member_info['weixin_info'] = serialize($weixin_info);
		
    

        //微信扫码登陆插件 end 

        //添加邀请人(推荐人)会员抵币 by shopjl.com

        $member_info['inviter_id'] = $register_info['inviter_id'];

        /* 微信三级返利插件start */

        $member_info['uid'] = $register_info['uid'];
    ...
    public function register_phone($register_info) {
        ...
        $insert_id = $this->addMember_phone($member_info);
    ...
    public function addMember_phone($param) {
    ...
    public function addMember($param) {
        ...
        try {
            ...
            $member_info['wecha_id'] = $param['wecha_id'];
            ...
            if ($param['uid']) {
                 ...
                 $user_tuijian['parent_uid'] = $param['uid'];



                $user_tuijian['uid'] = $insert_id;



                $user_tuijian['addtime'] = time();

                $this->table('user_tuijian')->insert($user_tuijian);
	...
	public function find_pwd($register_info){
        ...
        $obj_validate->validateparam = array(
            ...
            array("input" => $register_info["member_mobile"], "require" => "true", "message" => '用户名不能为空'),
        ...
        }else return array('error' => '手机号不存在');

		

        $condition = array();

        $condition['member_mobile'] = $register_info['member_mobile'];

		$data=array();

        $data['member_passwd'] = md5($register_info['member_passwd']);		

		

        $update	= $this->editMember($condition,$data);

		$result = $this->getMemberInfo($condition);
