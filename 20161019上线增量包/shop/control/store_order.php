...
class store_orderControl extends BaseSellerControl {
	...
	public function indexOp() {
        ...
        if ($_GET['self_code'] != '') {
			$list_order_id_search = model()->table('order_common')->field('order_id')->where(array('dlyo_pickup_code'=>$_GET['self_code']))->select();
			foreach($list_order_id_search as $v){
				$list_order_id[]=$v['order_id'];
			}
            $condition['order_id'] = array('in',$list_order_id);
        ...
        //$allow_state_array = array('state_new','state_pay','state_self','state_send','state_success','state_cancel');
        ...
        if (in_array($_GET['state_type'],$allow_state_array)) {
            ...
            $condition['order_state'] = str_replace($allow_state_array,
                    ...
                    array(ORDER_STATE_NEW,ORDER_STATE_PAY,ORDER_STATE_SELF,ORDER_STATE_SEND,ORDER_STATE_SUCCESS,ORDER_STATE_CANCEL), $_GET['state_type']);

        } 
        elseif ($_GET['state_type']=='state_lock') {
            $_GET['state_type'] = 'state_lock';
        }
        else {
        ...
        if($_GET['state_type']=='state_self'){$condition['shipping_type']=2;}elseif($condition['order_state']==20){$condition['shipping_type']=array('not in',2);}
        if($_GET['state_type']=='state_lock'){$condition['refund_state']=0;$condition['lock_state']=1;}
        ...
        foreach ($order_list as $key => $order_info) {
			if (!empty($_GET['self_code'])) {
				$order_info['self_code'] = $_GET['self_code'];
			}
		...
		$order_info['if_self'] = $model_order->getOrderOperateState('_self',$order_info);
	...
	public function show_orderOp() {
		...
		echo "<script>console.log(".json_encode($order_info).")</script>";
    ...
    private function profile_menu($menu_type='',$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            array('menu_key'=>'state_self',		    'menu_name'=>'到店自提',	    'menu_url'=>'index.php?act=store_order&op=index&state_type=state_self'),
            ...
            /*edit by peiyu start 增加一列退款中*/
                
            array('menu_key'=>'state_lock',	'menu_name'=>Language::get('nc_member_path_refund'),	'menu_url'=>'index.php?act=store_order&op=index&state_type=state_lock'),    
            /*edit by peiyu stop*/
