...
<div class="quick-login">
  <form id="login_form" action="<?php echo SHOP_SITE_URL;?>/index.php?act=login" method="post" class="bg" >
    ...
    <dl>
      <dt></dt>
      <dd>
          <input class="text" type="submit" value="登       录" style="width:258px;height:30px;background-color:#55a4e0;border:0;
">
      </dd>
    </dl>
    <ul>
    ...
    <div class="enter">
      <?php if ($output['setting_config']['qq_isuse'] == 1 || $output['setting_config']['sina_isuse'] == 1||$output['setting_config']['wexin_isuse'] == 1){?>
      <span class="other">
      ...
      <?php } ?>
            <!--<a class="weixin"  onclick="ajax_form('weixin_form', '微信账号登录', '<?php echo SHOP_SITE_URL?>/shop/index.php?act=connect_wx&op=index', 360);" href="javascript:void(0);">-->
            <a class="weixin"  onclick="ajax_form('weixin_form', '微信账号登录', '<?php echo SHOP_SITE_URL?>/index.php?act=connect_wx&op=index', 360);" href="javascript:void(0);">
      </span>
      <?php } ?> 
    </div>
