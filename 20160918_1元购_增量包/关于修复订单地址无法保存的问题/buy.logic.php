...
class buyLogic {
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        $store_voucher_list = $this->_logic_buy_1->getStoreAvailableVoucherList($store_goods_total, $member_id,$store_cart_list);
        ...
        list($need_calc_sid_list,$cancel_calc_sid_list,$store_free_price) = $this->_logic_buy_1->getStoreFreightDescList($store_goods_total);

        $result['need_calc_sid_list'] = $need_calc_sid_list;

        $result['cancel_calc_sid_list'] = $cancel_calc_sid_list;
		
		$result['store_free_price'] = $store_free_price;
    ...
    private function _createOrderStep1() {
		...
		}elseif($post['shipping_type']==2){
			
			$dlyo_pickup_code = rand(1,9).rand(0,9).rand(0,9).rand(0,9);
        ...
        if (!empty($post['voucher_0']) && is_array($post['voucher_0'])) {

            foreach ($post['voucher_0'] as $store_id => $voucher){
                     ...
                     if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)|(\d+)$/',$voucher,$matchs)) {
                        ...
                        if (floatval($matchs[3][0]) > 0) {
                            ...
                            $input_voucher_list[$store_id]['voucher_id'] = $matchs[4][1];
        ...
        $input_voucher_list_1 = array();

        if (!empty($post['voucher_1']) && is_array($post['voucher_1'])) {

            foreach ($post['voucher_1'] as $store_id => $voucher){      
  
                     if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)|(\d+)$/',$voucher,$matchs)) {

                        if (floatval($matchs[3][0]) > 0) {

                            $input_voucher_list_1[$store_id]['voucher_t_id'] = $matchs[1][0];

                            $input_voucher_list_1[$store_id]['voucher_price'] = $matchs[3][0];
                            
                            $input_voucher_list_1[$store_id]['voucher_id'] = $matchs[4][1];
        ...
        $input_voucher_list_2 = array();

        if (!empty($post['voucher_2']) && is_array($post['voucher_2'])) {

            foreach ($post['voucher_2'] as $store_id => $voucher){
                
                     if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)|(\d+)$/',$voucher,$matchs)) {                       

                       if (floatval($matchs[3][0]) > 0) {

                           $input_voucher_list_2[$store_id]['voucher_t_id'] = $matchs[1][0];

                           $input_voucher_list_2[$store_id]['voucher_price'] = $matchs[3][0];
                           
                           $input_voucher_list_2[$store_id]['voucher_id']    = $matchs[4][1];
        ...
        /*重组代金券*/
        $input_voucher_list_new = array();
        
        $input_voucher_list_new[39][]=$input_voucher_list[39];
        
        $input_voucher_list_new[39][]=$input_voucher_list_1[39];
       
        $input_voucher_list_new[39][]=$input_voucher_list_2[39];
        
        $input_voucher_list = $input_voucher_list_new;
	...
	$this->_order_data['dlyo_pickup_code'] = $dlyo_pickup_code;
	...
	$post = $this->_post_data;
        ...
        $input_voucher_list = $this->_logic_buy_1->reParseVoucherList($input_voucher_list,$store_goods_total,$this->_member_info['member_id'],$store_cart_list);
        ...
        //计算每个店铺运费
			...
			if($post['shipping_type']==2){
				foreach (array_keys($cancel_calc_sid_list) as $store_id) {
	
					$store_final_order_total[$store_id] = 0;
...
//         $this->_order_data['store_gc_id_commis_rate_list'] = $store_gc_id_commis_rate_list;
        ...
        //print_r($this->_order_data['input_voucher_list']);die;
	...
	$post = $this->_post_data;
	    ...
	    $order['shipping_type'] = $post['shipping_type']==2?2:1;

            $order['shipping_fee'] = $post['shipping_type']==1?$store_freight_total[$store_id]:0;
	    ...
	    $order_common['shipping_type'] = $post['shipping_type']==2?2:1;

            $order_common['store_id'] = $store_id;

            $order_common['order_message'] = $input_pay_message[$store_id];
			
	    $order_common['dlyo_pickup_code'] = $dlyo_pickup_code;
...
//                $order_common['voucher_code'] = $input_voucher_list[$store_id]['voucher_code'];
                ...
                $order_common['voucher_price'] = '';
             
                $order_common['voucher_code'] = '';  
                
                foreach($input_voucher_list as $voucher){
                    
                    foreach($voucher as $k){
                        
                        if(is_array($k)){
                            
                            $order_common['voucher_price'] = $order_common['voucher_price']."|".$k['voucher_price'];
                            
                            $order_common['voucher_code'] = $order_common['voucher_code']."|".$k['voucher_code'];
                            
                        }
                    }
                    
                }
                $order_common['voucher_price']=substr($order_common['voucher_price'],1);
                $order_common['voucher_code'] = substr($order_common['voucher_code'],1);
  
            }
            if($order_common['voucher_price']==''){
                $order_common['voucher_price']=0;
            }
            if($order_common['voucher_code']==''){
                $order_common['voucher_code']=0;
            }
            
           
            
            if($post['shipping_type'] == 1){
    ...
    private function _createOrderStep6() {
        ...
        if (!empty($input_voucher_list) && is_array($input_voucher_list)) {
           ...
           foreach( $input_voucher_list as $k =>$v){
               
               $input_voucher_list = array();
               
               foreach($v as $vo){
                   
                   $input_voucher_list[$k]=$vo;
