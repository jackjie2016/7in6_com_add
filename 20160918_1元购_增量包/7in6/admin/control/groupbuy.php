...
class groupbuyControl extends SystemControl{
    ...
    public function groupbuy_review_passOp(){
        ...
        $goods_data = $model_groupbuy->field('goods_id')->where(array('groupbuy_id'=>$groupbuy_id))->find();

        $update =  Model()->table('goods')->where(array('goods_id'=>$goods_data['goods_id']))->update(array('goods_promotion_type'=>1));
