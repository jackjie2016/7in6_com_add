    ...
    <div class="ncs-goods-summary">
      ...
      <div class="ncs-key">
        ...
        <dl>
          ...
          <dd class="ncs-figure-input">
            ...
            <?php if($output['goods']['purchase_quantity']>0){?>,已购<?php echo $output['groupbuy_info']['virtual_quantity']+$output['groupbuy_info']['buy_quantity']; ?>件<?php } ?>)			<?php if($output['goods']['purchase_quantity']>0){?>此商品每人限购 <?php echo $output['goods']['purchase_quantity'];?> 件 <?php }?>						</span><?php } ?>
          </dd>
        </dl>
        <?php }?>
        <!-- E 购买数量及库存 -->
      ...
      <!-- S 购买按钮 -->
        <div class="ncs-btn"><!-- S 提示已选规格及库存不足无法购买 -->
          ...
          <?php if ($output['goods']['cart'] == true  && empty($output['goods']['title'])) {?>
...
function checkSpec() {
    ...
    $.each(spec_param, function(i, n){
        if (n.sign == spec_sign) {			//var g_id = (n.url).substr((n.url).indexOf('goods_id') + 9);						$("#ck_goods_id").val(g_id);
           ...
           $(".price").html("<strong>¥"+price[0]+"</strong>");
		   var id=n.url.split("/");
